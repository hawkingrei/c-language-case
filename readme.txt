...
3-7.c 对字符变量赋值0，'\0'，'0'的区别
3-8.c 条件操作符的短路现象 
3-9.c 数组名当指针用
3-10.c 多维数组名当指针用
3-11.c 多维数组名当指针用(变化一)
3-12.c 多维数组名当指针用(变化二)错误实例
3-13.c 多维数组名当指针用(变化三)
...
5-9.c 指向函数的指针的实现
5-10.c 在函数中使用指向函数的指针
5-11.c *m[4]和(*n)[4],*(o[4])的区别
5-12.c 树的实现（前序，中序，后序）
5-13.c 队列的实现（添加，删除）
5-14.c *s+2和*(s+2)的区别
5-15.c 数组a的*[a+数字]遍历数组
5-16.c 函数指针int (*p)(int,int)演示

6-1.c 判断奇偶数
6-2.c 素数问题
6-3.c 选择排序法
6-4.c #真题#代码结果猜测（用指针实现函数返回数值）
6-5.c #真题#代码结果猜测（指针指向）
6-6.c /*判断输出结果（！用法，不要和！=搞混了）*/
6-7.c while (p--)的问题
6-8.c 转义字符\0的问题
